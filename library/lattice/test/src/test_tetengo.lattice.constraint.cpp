...
BOOST_AUTO_TEST_CASE(construction)
{
    ...
    }
    {
        ...
        const auto* const p_constraint = tetengo_lattice_constraint_create(pattern.data(), std::size(pattern));
        BOOST_SCOPE_EXIT(p_constraint)
        {
            tetengo_lattice_constraint_destroy(p_constraint);
        }
...
BOOST_AUTO_TEST_CASE(matches)
{
...

    {
...

        {
            ...
            BOOST_TEST(tetengo_lattice_constraint_matches(p_constraint, path.data(), std::size(path)));
        }
        {
    ...
    {
        const auto path = to_c_reverse_path(path_b_e());
        BOOST_TEST(!tetengo_lattice_constraint_matches(nullptr, path.data(), std::size(path)));
    }
    {
        const auto* const p_constraint = tetengo_lattice_constraint_createEmpty();
        BOOST_SCOPE_EXIT(p_constraint)
...
BOOST_AUTO_TEST_CASE(matches_tail_c)
{
...

    {
...

        {
            ...
            BOOST_TEST(tetengo_lattice_constraint_matchesTail(
                p_constraint, reverse_tail_path.data(), std::size(reverse_tail_path)));
        }
        {
    ...
    }
    {
        const auto reverse_tail_path = to_c_reverse_path(make_tail(path_b_e(), 1));
        BOOST_TEST(
            !tetengo_lattice_constraint_matchesTail(nullptr, reverse_tail_path.data(), std::size(reverse_tail_path)));
    }
    {
        const auto* const p_constraint = tetengo_lattice_constraint_createEmpty();
        BOOST_SCOPE_EXIT(p_constraint)
